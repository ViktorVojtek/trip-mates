...
- [ ] **4.1 Public trip listing** — remove the auth middleware from `GET /api/trips` and `GET /api/trips/:id` in `server/src/routes/trips.ts` so logged-out visitors can browse; keep `POST /`, `POST /:id/interest`, `GET /:id/interests` auth-gated. Update `server/src/routes/trips.test.ts` to assert both GETs are reachable without a token. (Fixes Landing showing "No trips posted yet" to visitors.)
- [ ] **4.2 `createdById` filter** — in `tripController.getTrips`, read `req.query.createdById` and add `if (createdById) where['createdById'] = createdById as string;`. Then remove the client-side re-filter workaround in `client/src/pages/PublicProfile.tsx`. Add a controller test for the filter.
